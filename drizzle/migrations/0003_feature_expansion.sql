-- Public/private visibility, participant cap for private auctions, item count,
-- owner lock, scheduled ("upcoming") auctions, listing currency, and the
-- ₹10L payment-acknowledgement gate on bids.
--
-- Idempotent: every statement is safe to re-run, including on a database
-- where this migration partially applied before. Run this whole file in one
-- go against your Postgres instance (Lovable Cloud / Supabase SQL runner).
...
-- Bids: what the bidder agreed to and the INR-equivalent snapshot used to decide
-- whether the acknowledgement was required. This is a policy gate, not a real
-- payment integration — no money moves through the database.
ALTER TABLE public.bids
  ADD COLUMN IF NOT EXISTS payment_ack boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS amount_inr numeric(14,2);

-- Private-auction participants: who's invited, who has actually joined (counts
-- toward the cap), set by the owner and accepted by the invitee.
...
-- Public auction feed must exclude private auctions now.
...
-- place_bid, extended: owner lock, scheduled/private gating, ₹10L payment ack.
